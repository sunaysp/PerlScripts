...
#$\ = "\n";
@array=`svn ls $branchURL/exchange`;
#print @array;

print "Checking out exchange project and AutomatedBuildScripts in temporary workspace folder\n";

`svn checkout --depth=files $branchURL/portal/AutomatedBuildScripts workspace/AutomatedBuildScripts`;

foreach(@array) {
	chomp($_);
	`svn checkout --depth=files $branchURL/exchange/$_ workspace/$_`;
	#print $_;
}

$dir="workspace";
print "Inside AutomatedBuildScripts and exchange folder... \nUpdating $srcURL with $branchURL\n";
find(\&wanted,$dir); # This line searches the srcURL in pom.xml, build.bat and build.properties file. Finally it updates with branchURL. 
